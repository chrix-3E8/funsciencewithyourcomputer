...
public class Chess2 {
	...
	String getNewBoard(String board, String move) {
		...
		if (isQueenSideCastlingMove(board, x1, y1, x2, y2))
			return getQueenSideCastling(board, x1, y1);
		if (isKingSideCastlingMove(board, x1, y1, x2, y2))
			return getKingSideCastling(board, x1, y1);
	...
	String[] getPossibleMovesKing(String board, int x, int y) {
		List result = new ArrayList();
		String castlingBoard = getQueenSideCastling(board, x, y);
		if (castlingBoard != null)
			result.add(castlingBoard);
		castlingBoard = getKingSideCastling(board, x, y);
		if (castlingBoard != null)
			result.add(castlingBoard);
		String piece = get(board, x, y);
		for (int i = -1; i <= 1; i++)
	...
	boolean isQueenSideCastlingMove(String board, int x1, int y1, int x2, int y2) {
		String piece = get(board, x1, y1);
		if (!isKing(piece))
			return false;
		if (y1 != y2 || (y1 != 0 && y1 != 8))
		...
		if (x1 != 4 || x2 != 2)
			return false;
		String piece1 = get(board, 0, y1);
		if (!isRook(piece1) || isDifferentColor(piece1, piece))
			return false;
		String piece2 = get(board, 1, y1);
		String piece3 = get(board, 2, y1);
		String piece4 = get(board, 3, y1);
		if (!isEmpty(piece2) || !isEmpty(piece3) || !isEmpty(piece4))
			return false;
		return true;
	}

	String getQueenSideCastling(String board, int x, int y) {
		if (!isQueenSideCastlingMove(board, x, y, 2, y))
			return null;
		String newBoard = move(board, x, y, 2, y);
		String newBoard2 = move(newBoard, 0, y, 3, y);
		return newBoard2;
	}

	boolean isKingSideCastlingMove(String board, int x1, int y1, int x2, int y2) {
		String piece = get(board, x1, y1);
		if (!isKing(piece))
			return false;
		if (y1 != y2 || (y1 != 0 && y1 != 8))
		...
		if (x1 != 4 || x2 != 6)
			return false;
		String piece1 = get(board, 7, y1);
		if (!isRook(piece1) || isDifferentColor(piece1, piece))
			return false;
		String piece2 = get(board, 5, y1);
		String piece3 = get(board, 6, y1);
		if (!isEmpty(piece2) || !isEmpty(piece3))
			return false;
		return true;
	}

	String getKingSideCastling(String board, int x, int y) {
		if (!isKingSideCastlingMove(board, x, y, 6, y))
			return null;
		String newBoard = move(board, x, y, 6, y);
		String newBoard2 = move(newBoard, 7, y, 5, y);
		return newBoard2;
	}
	...
	void blackMove() {
		...
		int depth = new Random().nextInt(3) + 1;
		boards = evalBest(boards, depth);
