...
public class PseudoRandom2 {
	...
	public static void main(String[] args) throws Exception {
		PseudoRandom2 random = new PseudoRandom2();
		long value = System.currentTimeMillis() % 100000000;
	...
	static class Screen extends Frame {
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < DICE_PER_ROW; i++) {
				for (int j = 0; j < DICE_PER_COLUMN; j++) {
					...
					drawLotto(g2, i, j, dice, value, isCurrentDice);
				}
			}
		...
		void drawLotto(Graphics g, int i, int j, int dice, long value,
				boolean isCurrentDice) {
			int width = 80, height = width, r = width / 3;
			int x = (width + r) * i + width;
			int y = (height + r) * j + height;
			g.setFont(new Font("Tahoma", Font.PLAIN, 32));
			if (isCurrentDice) {
				g.drawString("Lotto: " + dice + " (from calculated pseudo random value "
						+ value + ")", 0, 30);
				g.setColor(Color.ORANGE);
				g.fillRect(x - r, y - r, width + 2 * r, height + 2 * r);
			}
